...
const bookTimeSlots = async (child_id, parent_id, phone_number, check_in, check_out, schedule_pattern, time_slots) => {
    ...
    try {
        ...
        const bookingSql = `
            INSERT INTO bookings (childid, parentid, phonenumber, checkin, checkout, schedulepattern, timeslots, amount,payment_status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)
        `;
        const [insertid]=await pool.query(bookingSql, [child_id, parent_id, phone_number, check_in, check_out, schedule_pattern, JSON.stringify(time_slots), totalCost,payment_status]);
...
module.exports = {getAvailableHours, updateBookingCount, bookTimeSlots,getbookingByid,getbookingByparentid,getbookingByall,updatePayment};
